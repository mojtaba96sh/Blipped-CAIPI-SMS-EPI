...
slicesPerBand=Nz/mb;
bandStep = sliceThickness*slicesPerBand/2.5;
...
% ref EPI :  blip_dur = ceil(2*sqrt(deltak/sys.maxSlew)/sys.gradRasterTime/2)*sys.gradRasterTime*2;
% start with the blip
blip_dur = 2*ceil(2*sqrt(deltak(3)*bandStep/sliceThickness/sys.maxSlew)/sys.gradRasterTime/2)*sys.gradRasterTime*2;
...
for i=1:Ny % loop over phase encodes
    if i==1
        seq.addBlock(gx,gyBlip_up,mr.scaleGrad(gzBlip_up,1/(mb-1)),adc); % Read the first line of k-space with a single half-blip at the end
    elseif i==Ny
        seq.addBlock(gx,gyBlip_down,mr.addGradients({mr.scaleGrad(gzBlip_down,1/(mb-1)), mr.scaleGrad(gzBlip_up,-1)}, sys),adc); % Read the last line of k-space with a single half-blip at the beginning
    elseif (mod(i,mb)==1 && i~=1)
        seq.addBlock(gx,gyBlip_down_up,gzBlip_up_up,adc);
    elseif (mod(i,mb)==0 && i~=Ny)
        seq.addBlock(gx,gyBlip_down_up,gzBlip_down_down,adc);
    elseif (mod(i,mb)~=0 && mod(i,mb)~=1)
        seq.addBlock(gx,gyBlip_down_up,gzBlip_down_up,adc);
    end 
...
seq.setDefinition('Name', 'C');
seq.write('C.seq')       % Write to pulseq file
%seq.install('siemens');

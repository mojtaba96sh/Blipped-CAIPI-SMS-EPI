...
cmrr_kxkyzc_pos=squeeze(fft2c(LmyUtility.windows_filter_2d(ifft2c(MB_kxkyzsc(:,:,:,1,:)),[nFE nPE],@(x)tukeywin(x,0.2))));
cmrr_kxkyzc_neg=squeeze(fft2c(LmyUtility.windows_filter_2d(ifft2c(MB_kxkyzsc(:,:,:,2,:)),[nFE nPE],@(x)tukeywin(x,0.2))));
...
CSMweight_used=crop_new(CSMweight_xyz_epi_sakeCor,[nFE,nPE,size(CSMweight_xyz_epi_sakeCor,3)]);

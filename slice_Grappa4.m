...
% individual k-space
K_Indiv = {K1.*C1, K2.*C2, K3.*C3, K4.*C4};
...
shift_amount = 3*N(2)/MB_factor*[0,1,2,3];
for s = 1:MB_factor
    IMG_recon(:,:,:,s) = circshift( IMG_recon(:,:,:,s), [0,-shift_amount(s),0] );
end
...
figure();
subplot(221);imshow(rot90(squeeze(I(:,:,1)),1),[]);
subplot(222);imshow(rot90(squeeze(I(:,:,2)),1),[]);
subplot(223);imshow(rot90(squeeze(I(:,:,3)),1),[]);
subplot(224);imshow(rot90(squeeze(I(:,:,4)),1),[]);


% figure();
% subplot(221);imagesc(squeeze(I(:,:,1))); colorbar
% subplot(222);imagesc(squeeze(I(:,:,2))); colorbar
% subplot(223);imagesc(squeeze(I(:,:,3))); colorbar
% subplot(224);imagesc(squeeze(I(:,:,4))); colorbar
% 
